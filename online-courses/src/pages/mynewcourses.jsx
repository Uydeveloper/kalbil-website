...
export default function MyCourses() {
  ...
  useEffect(() => {
    ...
    if (loggedIn && savedUser) {
      ...
      const progress = progressData.find((p) => p.studentId === savedUser.id);
  ...
  // لوگىن بولۇش فۇنكسىيەسى - JSON مەلۇماتلىرىدىن تەكشۈرۈش
  const handleLogin = () => {
    // users.json دىن ئىشلەتكۈچىنى تەكشۈرۈش
    const user = usersData.find(u => 
      u.name === loginUsername && u.password === loginPassword
    );
    ...
    if (user) {
      ...
      const progress = progressData.find((p) => p.studentId === user.id);
      if (progress) setStudentProgress(progress.courses);
      setIsAdmin(user.role === 'admin');
    } else {
      setLoginError('ئىشلەتكۈچى ئىسمى ياكى مەخپى نومۇر خاتا');
    }
  };
  ...
  const subjectContents = {
    "stat101": [
      ...
      { title: "سىتاستىكا 7-باب - Variance, Standard Deviation, Correlation", file: "html/Variance · Standard Deviation · Correlation · Regression.html" },
    ...
    "stat102": [
      { "title": "پايسان 1-باب - كودلاش ئاساسى", "file": "پايسان/پايسان باشلانغۇچ.html" },
      { "title": "پايسان 2-باب - مۇھىت ۋە فۇنكسىيە", "file": "پايسان/پايسان ئىلغار تېمىلىرى.html" },
      ...
      <div className="min-h-screen bg-gradient-to-br from-purple-600 via-blue-600 to-indigo-700 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-md transform transition-all">
          <div className="text-center mb-8">
            ...
            <h2 className="text-3xl font-bold text-gray-800 mb-2">KeLBiL Akademiyasi</h2>
            <p className="text-gray-500">ئوقۇش سىستېمىسىغا خۇش كەلدىڭىز</p>
          </div>
          ...
          <div className="mt-6 text-center text-sm text-gray-500">
            <p>سىناق ئۇچۇرلىرى:</p>
            <p>KeLBiL / KeLBiLedu001 (باشقۇرغۇچى)</p>
            <p>Anwer Uyghur / Anwer-K0001 (ئوقۇغۇچى)</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="max-w-8xl mx-auto mt-4 p-4 sm:p-6 space-y-8 bg-gray-50 dark:bg-gray-900 min-h-screen">
      ...
      {/* ئىشلەتكۈچى ئۇچۇرى ۋە چىقىش كۇنۇپكىسى */}
      <div className="flex justify-between items-center bg-white dark:bg-gray-800 p-4 rounded-xl shadow-md">
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 bg-gradient-to-r from-cyan-500 to-amber-500 rounded-full flex items-center justify-center text-white font-bold text-xl">
            {currentUser?.name?.charAt(0) || "U"}
          </div>
          <div>
            <p className="font-semibold text-gray-800 dark:text-white">{currentUser?.name}</p>
            <p className="text-sm text-gray-500 dark:text-gray-400">
              {currentUser?.role === 'admin' ? '👑 باشقۇرغۇچى' : '🎓 ئوقۇغۇچى'} | {currentUser?.department || 'بۆلۈم'}
            </p>
          </div>
        </div>
        <button
      ...
      <div className="text-center mb-8 relative">
        <div className="flex justify-center mb-4">
          <img src={`${process.env.PUBLIC_URL}/images/image05.png`} alt="KaLBiL لوگوسى" className="w-auto h-20" />
        </div>
          ...
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-sm text-amber-700 dark:text-amber-300">
                ئىشلەتكۈچى: {currentUser?.name} | 
                رول: {currentUser?.role} |
                بۆلۈم: {currentUser?.department}
              </p>
            </div>
          </div>
        </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="p-4 flex flex-col flex-grow">
                    <h3 className="font-bold text-base sm:text-lg mb-2 text-gray-900 dark:text-white line-clamp-2">
                      {idx + 1}. {video.title}
                    </h3>
                    ...
                    <div className="mt-auto space-y-2">
                      <div className="grid grid-cols-2 gap-2">
                        <button
                          ...
                          className="px-2.5 py-2 bg-gradient-to-r from-cyan-500 to-blue-600 text-white text-xs sm:text-sm font-medium rounded-lg hover:from-cyan-600 hover:to-blue-700 transition-all shadow"
                        >
                          📖 تەپسىلى
                        </button>
                        <button
                          onClick={() => downloadHtmlAsPdf(notesFile, video.title)}
                          className="px-2.5 py-2 bg-gradient-to-r from-amber-500 to-orange-600 text-white text-xs sm:text-sm font-medium rounded-lg hover:from-amber-600 hover:to-orange-700 transition-all shadow"
                        >
                          📄 PDF
                        </button>
                      </div>
                      ...
                      <button
                        ...
                        className="w-full px-2.5 py-2 bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-200 text-xs sm:text-sm font-medium rounded-lg hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors"
                      >
                  ...
                  {expandedSections[sectionKey] && (
                    <div className="px-4 pb-4 mt-2 pt-2 border-t border-gray-100 dark:border-gray-700">
    ...
    <div className="max-w-5xl mx-auto mt-8 p-5 sm:p-6 bg-gradient-to-br from-cyan-50 to-amber-50 dark:from-gray-800 dark:to-gray-900 rounded-2xl shadow-lg border border-cyan-200 dark:border-gray-700 overflow-hidden">
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400 mt-1 text-right">
          دەرس ۋاقتى: ياپۇنىيە ۋاقتى (چارشەنبە، جۇمە،شەنبە: 22:00)
        </p>
      </div>
      ...
      <div className="overflow-x-auto rounded-xl border border-gray-200 dark:border-gray-700">
        <table className="w-full text-sm text-left text-gray-700 dark:text-gray-300 min-w-full">
          <thead className="text-xs uppercase bg-gradient-to-r from-cyan-500 to-amber-500 text-white">
            <tr>
              <th className="px-4 py-3">شەھەر</th>
              <th className="px-4 py-3">ۋاقىت (UTC)</th>
              <th className="px-4 py-3">يەرلىك ۋاقىت</th>
            </tr>
          </thead>
          <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
            {timeZones.map(({ city, offset, icon }) => (
              <tr key={city} className="hover:bg-cyan-50 dark:hover:bg-gray-700/50 transition-colors">
