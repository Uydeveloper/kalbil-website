...
export default function LoginForm({ onClose, onSuccess }) {
  ...
  const handleLogin = async (e) => {
    ...
    if (success) {
      ...
      setTimeout(() => {
        onSuccess?.(); 
        onClose?.();   
      className='font-uyghur text-xl'}, 800);
    } else {
  ...
  return (
    <div className="font-uyghur text-xl fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white dark:bg-gray-900 p-6 rounded-lg shadow-lg w-full max-w-md">
        <h2 className="font-uyghur text-xl font-bold mb-4 text-center text-gray-800 dark:text-white">
          🔐 Login
        </h2>
